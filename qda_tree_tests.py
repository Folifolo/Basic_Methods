...
class Branch(object):
    ...
    def grow(self):
        probs_train = self.qda.predict_proba(self.x)[:, 1]
        ...
        if min(right_y) == 1:
            ...
            qda_right = QuadraticDiscriminantAnalysis()
            qda_right.fit(right_x,right_y)
            m_right = find_optimal_param(qda_right, right_x, right_y)

        if max(left_y) == 0:
            qda_left = self.qda
            m_left = 1
        else:
            qda_left = QuadraticDiscriminantAnalysis()
            qda_left.fit(left_x,left_y)
            m_left = find_optimal_param(qda_left, left_x, left_y)

        right_branch = Branch(qda_right, m_right, right_x, right_y)
        left_branch = Branch(qda_left, m_left, left_x, left_y)
    ...
    def divide_data(self, x, y):
        probs_train = self.qda.predict_proba(x)[:, 1]
    ...
    def predict(self, x, y):
        ...
        if self.left == None:
            if len(x) != 0:
                probs= self.qda.predict_proba(x)[:, 1]
...
if __name__ == "__main__":
    ...
    for d in MOST_FREQ_DIAGS_NUMS:
        ...
        for train_index, test_index in cross_val(b.shape[0], 1):
            ...
            m = find_optimal_param(qda, b[train_index, :num_components], Y[train_index,d])

            tree = Branch(qda, m, b[train_index, :num_components], Y[train_index,d])
            tree.grow()
