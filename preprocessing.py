...
def make_thresh(x, sup, inf):
    ...
    for i in sup:
        x_new[i] = x[i]
        j = i
        while x_filtred[j] >= x_filtred[min(j+1,x.shape[0]-1) ] and x_filtred[min(j+1,x.shape[0]-1)] > 10  and j < x.shape[0]-1 :
            x_new[j+1] = x_new[i]
            j += 1
        j = i
    ...
    for i in inf:
        x_new[i] = x[i]
        j = i
        while x_filtred[j] <= x_filtred[min(j+1,x.shape[0]-1)] and x_filtred[min(j+1,x.shape[0]-1)] < -5  and j < x.shape[0]-1 and x_new[j+1] ==0:
            x_new[j+1] = x_new[i]
            j += 1
        j = i
        while x_filtred[j] <= x_filtred[max(j-1,0)] and x_filtred[max(j-1,0)] < -5 and j > 0 and x_new[j-1] ==0:
            x_new[j-1] = x_new[i]
            j -= 1
